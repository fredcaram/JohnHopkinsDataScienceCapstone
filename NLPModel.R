...
stupid_backoff <- function(prewords, kgrams, maxkgram, number_of_recomendations = 3){
...
simple_interpolation <- function(prewords, kgrams, maxkgram, number_of_recomendations = 3){
  prewords_corp <- tm.GetTextCorpus(prewords)
  prewords_corp <- tm.CleanCorpus(prewords_corp)
  prewords_content <- prewords_corp[[1]]$content
  ...
  while(k > 1){
    ...
    if(length(found_elements) > 0){
      ...
      p <- data.table(word=names(found_elements), prob=(found_elements / total_count), weight=(1 - (maxkgram + 1 - k) * 0.2 ))
...
discounting <- function(kgram){
  i <- 4;
  freqtable <- table(kgram);
  while(i > 0){
    i <- i - 1;
  }
}

kneser_ney <- function(prewords, kgrams, maxkgram, number_of_recomendations = 3){
  prewords_corp <- tm.GetTextCorpus(prewords)
  prewords_corp <- tm.CleanCorpus(prewords_corp)
  prewords_content <- prewords_corp[[1]]$content
